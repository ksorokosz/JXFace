...
public class MorphBlender
{
	...
	private void computeEntityDifference(final Entity rest, Entity dst, final Entity ent)
	{
		...
		{
			Drawable draw_dst = it_dst.next();
			Drawable draw_ent = it_ent.next();
			Drawable draw_rest = it_rest.next();
			DeformableGeometry pDst = pMM.getMesh(draw_dst.getMeshName());
			DeformableGeometry pEnt = pMM.getMesh(draw_ent.getMeshName());
			DeformableGeometry pRest = pMM.getMesh(draw_rest.getMeshName());
	...
	public Entity prioritizeChannel(final Hashtable<String, Entity> dictionary, final Entity rest)
	{
		...
		{
			if(!e.getKey().equals("Rest") && !e.getKey().equals("#")) // rest and silence period
			{
